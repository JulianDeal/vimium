const DomUtils = {
  ...
  // Returns a dialog element if there is one.
  getDialogElement(element) {
    return document.getElementsByTagName("dialog")[0] || null;
  },

  getParentByTagName(tagName, element) {
    if (tagName !== null)
      tagName.toLowerCase()
    while (element && element.parentNode) {
      element = element.parentNode;
      elementTagName = element.tagName && element.tagName.toLowerCase();
